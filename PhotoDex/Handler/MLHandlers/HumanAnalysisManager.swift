...
class HumanAnalysisManager {
    ...
    private let resultsQueue = DispatchQueue(label: "com.PhotoDex.resultsQueue")
    
    //MARK: - Initialisation of the models and the requests
    private init() {
    ...
    func analyzeHuman(in image: UIImage, completion: @escaping (HumanAnalysisResult?) -> Void) {
        ...
        dispatchGroup.notify(queue: .main) {
            ...
            self.resultsQueue.sync {
                age = self.ageResults
                gender = self.genderResults
                emotion = self.emotionResults
            }
            
            if let age = age, let gender = gender, let emotion = emotion {
